      ...
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          <div class="col-md-6 col-md-offset-3">
            <form action="<?= base_url('admin/data_buku/tambah') ?>" method="POST">
              <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
              <input type="hidden" name="id_buku" id="id_buku" value="<?= $uuid ?>">
              <div class="form-group">
                <label for="kode_buku">Kode Buku</label>
                <input name="kode_buku" id="kode_buku" type="text" class="form-control" readonly="" value="<?= $kodebuku ?>">
              </div>
              ...
              <div class="form-group <?= form_error('isbn') ? 'has-error' : null ?>">
                ...
                <input name="isbn" id="isbn" type="text" class="form-control" value="<?= set_value('isbn') ?>">
              ...
              <div class="form-group <?= form_error('penerbit') ? 'has-error' : null ?>">
                ...
                <input name="penerbit" id="penerbit" type="text" class="form-control" value="<?= set_value('penerbit') ?>">
              ...
              <div class="form-group <?= form_error('id_lokasi') ? 'has-error' : null ?>">
                ...
                <select name="id_lokasi" id="id_lokasi" class="form-control select2">
                  ...
                  <?php foreach ($lokasi as $value): ?>
                    <option value="<?= $value['id_lokasi'] ?>" <?= set_select('id_lokasi', $value['id_lokasi']) ?>><?= strtoupper($value['kode_lokasi']) ?></option>
                  <?php endforeach ?>
                </select>
              ...
              <div class="form-group <?= form_error('jumlah') ? 'has-error' : null ?>">
                ...
                <input name="jumlah" id="jumlah" type="number" class="form-control" value="<?= set_value('judul') ?>">
